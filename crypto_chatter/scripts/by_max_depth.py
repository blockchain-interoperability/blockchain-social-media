...
import pandas as pd
if not pd.Series(graph.nodes).isin(data.df['id'].astype(int)).all():
    print('nodes not in data')
    quit()
...
# task = progress.add_task('aggregating per depth..', total=len(list(neighbor_by_depth.keys())))
