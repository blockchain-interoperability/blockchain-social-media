...
from crypto_chatter.graph import CryptoChatterGraphBuilder, CryptoChatterGraph
...
@click.command()
@click.argument("user_graph_kind")
@click.option("--top_n", default=10)
def subgraph_stats(
    user_graph_kind: Literal["reply", "quote"],
    top_n: int,
) -> None:
    ...
    graph_kind = f"user-{user_graph_kind}"
