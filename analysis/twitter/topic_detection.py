...
def get_bert_embeddings(tokens_path = '',bert_embeddings_path = '',):
    ...
    if not bert_embeddings_file.is_file():
        ...
        embeddings = model.encode(list(map(lambda x: ' '.join(x),tokens)), show_progress_bar=True)
...
def get_umap_embeddings(tokens_path = '',bert_embeddings_path='',umap_embeddings_path = ''):
